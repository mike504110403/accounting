...
void main() {
  test('LedgerNotifier.update 覆寫 ledger 狀態', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    final before = container.read(ledgerProvider);
    container.read(ledgerStateProvider.notifier).update(
  ...
  test('MembersNotifier.update 只替換對應 id', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    final me = container.read(membersProvider).firstWhere((m) => m.id == kMeId);
    container.read(membersStateProvider.notifier).update(
  ...
  test('CategoriesNotifier.add/update/remove', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    final notifier = container.read(categoriesStateProvider.notifier);
    final before = container.read(categoriesProvider).length;

    notifier.add(const Category(id: 'c-test', ledgerId: kLedgerId, kind: EntryKind.expense, name: '測試', icon: 'more_horiz', sort: 99));
    expect(container.read(categoriesProvider).length, before + 1);

    notifier.update(const Category(id: 'c-test', ledgerId: kLedgerId, kind: EntryKind.expense, name: '測試改名', icon: 'more_horiz', sort: 99, rollover: true));
  ...
  test('CategoriesNotifier.reorder 只影響同 kind、依 sort 重排', () {
    ...
    notifier.reorder(EntryKind.expense, 0, 2);
  ...
  test('CategoriesNotifier.reorder 寫回前已依 kind、sort 統一排序（不靠消費端再 sort）', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    container.read(categoriesStateProvider.notifier).reorder(EntryKind.expense, 0, 2);
  ...
  test('CategoriesNotifier.reorder 索引越界時不動 state', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    final before = container.read(categoriesProvider);

    container.read(categoriesStateProvider.notifier).reorder(EntryKind.expense, -1, 2);
    expect(container.read(categoriesProvider), before);

    container.read(categoriesStateProvider.notifier).reorder(EntryKind.expense, 0, 999);
    expect(container.read(categoriesProvider), before);

    container.read(categoriesStateProvider.notifier).reorder(EntryKind.income, 5, 0); // 收入只有 2 筆
    expect(container.read(categoriesProvider), before);
  });
}
