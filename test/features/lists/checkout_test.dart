...
void main() {
  ...
  group('dueStatus', () {
    ...
    test('無到期日 → none', () {
      expect(dueStatus(null, now), DueStatus.none);
    });

    test('已過期 → overdue', () {
      expect(dueStatus(DateTime(2026, 9, 1), now), DueStatus.overdue);
    });

    test('3 天內（含今天）→ soon', () {
      expect(dueStatus(DateTime(2026, 9, 2), now), DueStatus.soon);
      expect(dueStatus(DateTime(2026, 9, 5), now), DueStatus.soon);
    });

    test('超過 3 天 → normal', () {
      expect(dueStatus(DateTime(2026, 9, 6), now), DueStatus.normal);
    });
  });
}
